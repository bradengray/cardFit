...
- (CGSize)adjustedCardSize {
    if ([self defaultCardSize].height > [self frameSize].height - MINIMUM_VOFFSET) {
        CGFloat sizeAdjustment = [self defaultCardSize].height - ([self frameSize].height - MINIMUM_VOFFSET);
...
- (CGFloat)voffset {
    CGFloat offset = ([self frameSize].height - [self adjustedCardSize].height) / 2.0;
